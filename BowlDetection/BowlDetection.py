...
def vis(preds, res_img, letterbox_scale, fps=None):
    ...
    for pred in preds:
        ...
        if classes[classid] == 'toilet':  # check if the detected object is "toilet"
            # bbox
            ...
            # Fit an ellipse to the bounding box area
            gray = cv.cvtColor(ret, cv.COLOR_BGR2GRAY)
            roi_gray = gray[ymin:ymax, xmin:xmax]
            blurred = cv.GaussianBlur(roi_gray, (5, 5), 0)
            ...
            for contour in contours:
                if len(contour) >= 5:  # Minimum number of points required to fit an ellipse
                    ...
                    if ellipse[1][0] >= 0 and ellipse[1][1] >= 0:
                        ...
                        ellipse = ((ellipse[0][0] + xmin, ellipse[0][1] + ymin), ellipse[1], ellipse[2])
                        cv.ellipse(ret, ellipse, (0, 255, 0), 2)
                        
                        # Draw major axis extending beyond the ellipse
                        major_axis_length = 0.8 * ellipse[1][0]  # Adjust the scaling factor here
                        angle_rad = np.deg2rad(ellipse[2])
                        center = (int(ellipse[0][0]), int(ellipse[0][1]))
                        endpoint_x = int(center[0] + 0.5 * major_axis_length * np.cos(angle_rad))
                        endpoint_y = int(center[1] + 0.5 * major_axis_length * np.sin(angle_rad))
                        cv.line(ret, center, (endpoint_x, endpoint_y), (255, 0, 0), 2)
                        
                        # Draw minor axis
                        minor_axis_length = ellipse[1][1]
                        angle_rad = np.deg2rad(ellipse[2] + 90)
                        endpoint_x = int(center[0] + 0.5 * minor_axis_length * np.cos(angle_rad))
                        endpoint_y = int(center[1] + 0.5 * minor_axis_length * np.sin(angle_rad))
                        cv.line(ret, center, (endpoint_x, endpoint_y), (0, 0, 255), 2)
                        
                        # Calculate major and minor axes ratio
                        ratio = major_axis_length / minor_axis_length
                        
                        # Display ratio near the ellipse
                        text = f"Ratio: {ratio:.2f}"
                        cv.putText(ret, text, (int(ellipse[0][0] - 50), int(ellipse[0][1] + 50)), cv.FONT_HERSHEY_SIMPLEX, 
